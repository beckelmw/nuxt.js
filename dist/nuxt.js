...
class Builder {
  ...
  get plugins() {
    return ___default.uniqBy(this.options.plugins.map((p, i) => {
      if (typeof p === 'string') p = { src: p };
      return {
        src: this.nuxt.resolvePath(p.src),
        ssr: p.ssr !== false,
        name: path.basename(p.src, path.extname(p.src)).replace(/[^a-zA-Z?\d\s:]/g, '') + '_' + hash(p.src)
      };
    }), p => p.name);
  }
  ...
  generateConfig() {
    ...
    return asyncToGenerator(function* () {
      const config = path.resolve(_this5.options.buildDir, 'build.config.js');
      const options = ___default.omit(_this5.options, Options.unsafeKeys);
      yield fs$1.writeFile(config, `module.exports = ${JSON.stringify(options, null, '  ')}`, 'utf8');
    })();
  }
}
